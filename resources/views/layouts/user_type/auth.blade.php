...
@section('auth')
    ...
    @else
        ...
        @else
            @include('layouts.navbars.auth.sidebar')
            <main class="main-content position-relative mt-1 border-radius-lg {{ (Request::is('rtl') ? 'overflow-hidden' : '') }}">
                @include('layouts.navbars.auth.nav')
                <div class="container-fluid py-4">
                    @yield('content')
                </div>
...
<body>
    {{-- ...existing code... --}}
    <script src="{{ asset('assets/js/your-theme.js') }}"></script>
    {{-- Tambahkan semua JS yang dibutuhkan dengan asset() --}}
    @stack('scripts')
</body>
</html>